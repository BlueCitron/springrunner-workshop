...
public class ReadableErrorAttributes implements ErrorAttributes, HandlerExceptionResolver, Ordered {
    ...
    public Map<String, Object> getErrorAttributes(WebRequest webRequest, boolean includeStackTrace) {
        ...
        if (Objects.nonNull(error)) {
        	String newMessage = String.valueOf(attributes.get("message"));
        	if (error instanceof MessageSourceResolvable) {
        		newMessage = messageSource.getMessage(
        				(MessageSourceResolvable) error, webRequest.getLocale());
        	} else {
        		// ex) Exception.MethodArgumentNotValidException
        		String code = String.format(
        				"Exception.%s", error.getClass().getSimpleName());
            	newMessage = messageSource.getMessage(
            			code, new Object[0], error.getMessage(), webRequest.getLocale());        		
        	}        	
        	attributes.put("message", newMessage);
        }
